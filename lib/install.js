...
function download(uri,opts,callback) {
    ...
    if (req) {
        req.on('response', function (res) {
            ...
            var stream = highland(req);
            cache.write_to_cache(stream.observe(), opts);
            callback(null,stream.fork());
        });
    }
}
...
function extract(source,to,callback) {
    ...
    function extract_log(entry) {
        ...
        if(extracted === false) extracted = true;
    }
    ...
    extracter.on('error', after_tarball);
    extracter.on('entry', extract_log);
    extracter.on('finish', after_tarball);
